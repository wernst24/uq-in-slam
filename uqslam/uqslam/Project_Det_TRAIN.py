...
def main():
    ...
    for x in range(1, total_episodes, 1):
        ...
        mem_observation = np.zeros(
            (batch_sz, img_rows, img_cols, img_channels))
        ...
        for i in range(steps):  # <----- # of steps
            ...
            actions[batch_step], values[batch_step] = agent.action_value(
                observation)
            newObservation, rewards[batch_step], dones[batch_step], _, info = env.step(
                actions[batch_step])
            ...
            if (batch_step == 15):  # <---- batch size: learn after 250 steps
                if (i > 250):
                    ...
                    returns, advs = agent._returns_advantages(
                    ...
                    # actor_loss function
                    losses = agent.model.train_on_batch(
                        mem_observation, [acts_and_advs, returns])
            ...
            if (dones[batch_step]):
                ...
                if last100RewardsIndex >= 100:
                    ...
                    last100RewardsIndex = 0
                m, s = divmod(
                    int(time.time() - start_time + loadsim_seconds), 60)
                h, m = divmod(m, 60)
                if not last100Filled:
                    print("EP "+str(x)+" - {} steps".format(i+1)+" - CReward: "+str(round(cumulated_reward, 2)
                                                                                    )+"  Eps="+str(round(explorationRate, 2))+"  Time: %d:%02d:%02d" % (h, m, s))
                else:
                    print("EP "+str(x)+" - {} steps".format(i+1)+" - last100 C_Rewards : "+str(int((sum(last100Rewards)/len(last100Rewards)))) +
                          " - CReward: "+str(round(cumulated_reward, 2))+"  Eps="+str(round(explorationRate, 2))+"  Time: %d:%02d:%02d" % (h, m, s))
        ...
        if x % 50 == 0:
            ...
            agent.model.save_weights(
                '/tmp/turtle_c2c_Actor_ep'+str(x)+"_train.weights.h5")

            # copy_tree(outdir,'/tmp/turtle_c2c_dqn_ep'+str(x))
            # save simulation parameters.
            parameter_keys = ['explorationRate', 'minibatch_size', 'learnStart', 'learningRate', 'discountFactor', 'memorySize',
                              'network_outputs', 'current_epoch', 'stepCounter', 'EXPLORE', 'INITIAL_EPSILON', 'FINAL_EPSILON', 'loadsim_seconds']
            parameter_values = [explorationRate, minibatch_size, learnStart, learningRate, discountFactor,
                                memorySize, network_outputs, x, i, EXPLORE, INITIAL_EPSILON, FINAL_EPSILON, s]
            parameter_dictionary = dict(zip(parameter_keys, parameter_values))
            with open('/tmp/turtle_c2c_dqn_ep'+str(x)+'.json', 'w') as outfile:
                json.dump(parameter_dictionary, outfile)

            # PLOT
            # list( range(50) ) #for total number of episodes/epochs
            x_plot = range(x)
            x_plot_avg.append(x)
        ...
        node.get_logger().debug("[%d/%d] Losses: %s" %
                                (x + 1, total_episodes, losses))
